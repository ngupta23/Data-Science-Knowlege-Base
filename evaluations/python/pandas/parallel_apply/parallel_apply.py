...
def apply_grouped_parallel_with_args(
    ...
    **kwargs: optional
    ...
    # If the function is returning multiple Data frames, then the object will be a Tuple of DataFrames
    ...
    if isinstance(ret_list[0], tuple):
        ...
        def unpack(col, index):
            # If the initial unpack returns a series of DataFrames (one data frame for each group)
            # then we need to further unpack the data frames and concatenate them together
        ...
        initial_unpack = [
            return_obj.apply(return_col, col_index=j) for j in range(len(return_obj[0]))
        ]
    ...
    raise (
        TypeError(
            "Currently only supports functions that return a Pandas DataFrame, Pandas Series or a Tuple of values"
        )
    )
