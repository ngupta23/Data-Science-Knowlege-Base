...
def apply_grouped_parallel_with_args(
    ...
    **kwargs: optional
    ...
    if isinstance(ret_list[0], pd.DataFrame):
        ...
        return_obj = pd.concat(ret_list)
        return_obj.index = indices
        return return_obj 
    # Function 'func' is returning 1 Series per group
    elif isinstance(ret_list[0], pd.Series):
        return_obj = pd.concat(ret_list, axis=1).T
        return_obj.index = indices
        return return_obj  
    # Function 'func' is returning a tuple of values per group
    # Each value could be a DataFrame, Series or something else
    elif isinstance(ret_list[0], tuple):
